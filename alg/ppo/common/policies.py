...
from .utils import conv, linear, conv_to_fc, batch_to_seq, seq_to_batch, lstm
...
def nature_cnn(scaled_images, **kwargs):
    ...
    net = tf.nn.relu(linear(_cnn(scaled_images), 'fc1', n_hidden=128, init_scale=np.sqrt(2)))
    net = tf.nn.relu(linear(net, 'fc2', n_hidden=128, init_scale=np.sqrt(2)))
    return net

from functools import partial
conv2_ = partial(tf.layers.conv2d, activation=tf.nn.relu,
...
def _cnn(x):
    ...
    net = max_pool(conv2_(x, filters=64, kernel_size=3, strides=2, name='conv2_1'))

    net = max_pool(conv2_(net, filters=128, kernel_size=4, strides=1, name='conv2_2'))

    net_max = max_pool(conv2_(net, filters=64, kernel_size=3, strides=2, name='conv2_3'))
    ...
    # 合起来
    return tf.concat([tf.layers.flatten(net),
